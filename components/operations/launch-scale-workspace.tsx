...
const integrationLabels: Array<{ key: keyof LaunchSnapshot["integrations"]; label: string; detail: string; icon: typeof Mail }> = [
  ...
  { key: "googleCalendar", label: "Google Calendar", detail: "OAuth-backed study-plan synchronization", icon: CalendarSync },
...
export function LaunchScaleWorkspace({ userId }: { userId: string }) {
  ...
  async function saveReleaseControls(event: FormEvent<HTMLFormElement>) {
    ...
    await run("controls", async () => {
      ...
      const connectionRows = await Promise.all((["google"] as const).map((provider) => tables.upsertRow<CalendarConnection>({ databaseId: config.databaseId, tableId: "calendar_connections", rowId: `${userId}-${provider}`, data: { ownerId: userId, provider, status: "not-configured", syncMode: "export", conflictPolicy, updatedAt: now }, permissions })));
